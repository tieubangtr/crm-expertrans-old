<?php
    ...
    $index = $start + 1;
    if($type == "none"){
        $sql = "select c.*, u1.name as nguoitao_name, u2.name as nguoiphutrach_name from customer c, users u1, users u2 where c.nguoitao = u1.id and c.nguoiphutrach = u2.id order by id desc limit ".$start.", ".$record_per_page.";";
    }else{
        $sql = "select c.*, u1.name as nguoitao_name, u2.name as nguoiphutrach_name from customer c, users u1, users u2 where c.nguoitao = u1.id and c.nguoiphutrach = u2.id and tinhtrang = '$type' order by id desc limit ".$start.", ".$record_per_page.";";
    }
