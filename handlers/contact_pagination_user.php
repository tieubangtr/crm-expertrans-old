<?php
    ...
    $index = $start + 1;
    if($type == "none"){
        $sql = "select c.*, u.name as nguoitao_name from customer c, users u where c.nguoitao = u.id and nguoitao = '$id' order by id desc limit ".$start.", ".$record_per_page.";";
    }else{
        $sql = "select c.*, u.name as nguoitao_name from customer c, users u where tinhtrang = '$type' and c.nguoitao = u.id and nguoitao = '$id' order by id desc limit ".$start.", ".$record_per_page.";";
    }
    ...
    while($row = mysqli_fetch_assoc($result)){
        $output .= "<tr>
                        ...
                        <td>
                            <button type='button' class='btn btn-sm btn-outline-primary editContact' style='margin-left: 10px;' id='edit-".$row['id']."'>Sửa</button>
                            <button type='button' class='btn btn-sm btn-outline-danger removeContact' style='margin-left: 10px;' id='remove-".$row['id']."'>Xóa</button></td>
                    </tr>
                    ";
        $index++;
    }
